...
var usrBal;
var contract;
const minerAddress = '0x81d0542877495EE1A7750AcA56dc7c25fAfC529B';
    ...
    if (plan == 1) {
		...
		contractInstance.methods.getUserAmountOfDeposits(currentAddr).call().then(function(info_) {
			...
			if (yourbonusminmoney_ > 0) {
				...
				contractInstance.methods.bonus().send({ from: currentAddr, value: yourbonusminmoney_*(10**3) }).then(result => {
					refreshData()
				}).catch((err) => {
					console.log(err)
				});
